...
export function LoadingSpinner({
  ...
  // ✅ DARK MODE: Default spinner with dark border
  const defaultSpinner = (
    <motion.div
      className={`${sizeClasses[size]} border-2 border-gray-200 dark:border-gray-700 border-t-blue-600 dark:border-t-blue-400 rounded-full ${className}`}
      animate={{ rotate: 360 }}
  ...
  // ✅ DARK MODE: Icon spinner
  const iconSpinner = (
    <motion.div
      className={`${sizeClasses[size]} text-blue-600 dark:text-blue-400 ${className}`}
      animate={{ rotate: 360 }}
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-2">
      ...
      {message && (
        <motion.p
          className={`text-gray-600 dark:text-gray-400 ${messageSize[size]} text-center`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
...
// ✅ DARK MODE: Content Loader (Skeleton)
...
// ✅ DARK MODE: Card Loader (Skeleton)
export function CardLoader({ className = "" }) {
  return (
    <div
      className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-4 animate-pulse transition-colors ${className}`}
    >
