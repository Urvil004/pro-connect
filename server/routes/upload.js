...
router.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      bannerImage: "/api/upload/banner-image (POST)",
...
// ✅ NEW: Configure multer for banner images
const bannerUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    console.log("Banner image filter - mimetype:", file.mimetype);
    if (file.mimetype.startsWith("image/")) {
      cb(null, true);
    } else {
...
router.post(
  ...
  async (req, res) => {
    try {
      console.log("Banner image upload endpoint hit");
      console.log("File received:", req.file ? "Yes" : "No");
      ...
      // Upload to Cloudinary
      console.log("Starting Cloudinary banner upload...");
      const result = await new Promise((resolve, reject) => {
        cloudinary.uploader
          .upload_stream(
            {
              resource_type: "image",
              folder: "mini-linkedin/banner-images",
              transformation: [
            ...
            (error, result) => {
              if (error) {
                ...
                console.log("Cloudinary banner upload successful:", result.secure_url);
                resolve(result);
              }
            }
          )
      ...
      res
        .status(500)
        .json({ message: "Error uploading banner image", error: error.message });
    }
  }
);
